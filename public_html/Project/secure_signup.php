...
<body>
	...
	if($_SERVER["REQUEST_METHOD"]=="POST") {
		...
		return $data;
			...
			/* gotcha4 hashing the user's password */
		...
		//MySQL Magic :D
			...
			include 'mysql.php';
        		if($resid) {
			$check_email=MySQLi_Query($resid,"select name from hash_students where email='".$email."'");
			...
			$count=MySQLi_Query($resid,"select (max(id)+1) as count  from hash_students");
			$count_id=MySQLi_Fetch_Assoc($count);
			if($count_id["count"]) {
				$query="insert into hash_students values (".$count_id["count"].",'$name','$email',$age,'$gender','$hash')";
			}
			else {
				$query="insert into hash_students values (1,'$name','$email',$age,'$gender','$hash')";
			}
			$res=MySQLi_Query($resid,$query);
			if($res) {
			echo "<tr align='center'> <td colspan='5'> <font color='lightgreen'> Registration Successful! </font> You may login now from here:- <a href='login.php'>Login</a></td> </tr>";
			}
			else {
